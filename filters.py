...
`CloseApproach`) constructed from a comparator (from the `operator`
module), a reference value, and a class method `get` that subclasses
can override to fetch an attribute of interest from the supplied
`CloseApproach`.
...
class AttributeFilter:
    ...
    comparing some attribute of a close approach (or its attached
    NEO) to a reference value. It essentially functions as a
    callable predicate for whether a `CloseApproach` object
    satisfies the encoded criterion.
...
class DateFilter(AttributeFilter):
    """Filter by exact date of close approach."""
...
class StartDateFilter(AttributeFilter):
    """Filter by start date of close approach."""
...
class EndDateFilter(AttributeFilter):
    """Filter by end date of close approach."""
...
class DistanceMinFilter(AttributeFilter):
    """Filter by minimum approach distance."""
...
class DistanceMaxFilter(AttributeFilter):
    """Filter by maximum approach distance."""
...
class VelocityMinFilter(AttributeFilter):
    """Filter by minimum approach velocity."""
...
class VelocityMaxFilter(AttributeFilter):
    """Filter by maximum approach velocity."""
...
class DiameterMinFilter(AttributeFilter):
    """Filter by minimum NEO diameter."""
...
class DiameterMaxFilter(AttributeFilter):
    """Filter by maximum NEO diameter."""
...
class HazardousFilter(AttributeFilter):
    """Filter by whether the NEO is potentially hazardous."""

    @classmethod
    def get(cls, approach):
        """Get the hazardous flag of the associated NEO."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    option corresponds to the `distance_min` argument, and
    represents a filter that selects close approaches whose nominal
    approach distance is at least that far away from Earth. Each
    option is `None` if not specified at the command line (in
    particular, this means that the `--not-hazardous` flag results
    ...
    if distance_min is not None:
        filters.append(DistanceMinFilter(operator.ge, distance_min))
    if distance_max is not None:
        filters.append(DistanceMaxFilter(operator.le, distance_max))
    if velocity_min is not None:
        filters.append(VelocityMinFilter(operator.ge, velocity_min))
    if velocity_max is not None:
        filters.append(VelocityMaxFilter(operator.le, velocity_max))
    if diameter_min is not None:
        filters.append(DiameterMinFilter(operator.ge, diameter_min))
    if diameter_max is not None:
        filters.append(DiameterMaxFilter(operator.le, diameter_max))
