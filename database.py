...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        (`._designation`) that matches the `.designation` attribute
        of the corresponding NEO. This constructor modifies the
        supplied NEOs and close approaches to link them together -
        after it's done, the `.approaches` attribute of each NEO has
        a collection of that NEO's close approaches, and the `.neo`
        attribute of each close approach references the appropriate
        NEO.
        ...
        self._designation_map = {neo.designation: neo for neo in self._neos}
        self._name_map = {}
        for neo in self._neos:
            if neo.name:
                self._name_map[neo.name] = neo

        for approach in self._approaches:
            neo = self._designation_map.get(approach._designation)
            if neo:
                approach.neo = neo
